"""
ML-Powered Adaptive Lesson Quizzes
- Intelligent question selection based on difficulty progression
- Performance tracking by topic and difficulty level
...
# Select 5 questions adaptively based on student's past performance
def select_adaptive_questions(lesson_key, all_questions, username):
    """
    ML Algorithm: Adaptive Question Selection
    - Analyzes past performance
    - Starts with easier questions
    - Adapts difficulty based on success rate
    """
    # Get user's past performance for this lesson
    ...
    # Determine difficulty distribution based on past performance
    past_score = lesson_history.get('best_score_pct', 0)
    
    if past_score == 0:  # First attempt
        # Start easier: 2 easy, 2 medium, 1 hard
    ...
    elif past_score < 60:  # Struggling
        # More easy/medium: 2 easy, 2 medium, 1 hard
        selected.extend(random.sample(easy_q, min(2, len(easy_q))))
        selected.extend(random.sample(medium_q, min(2, len(medium_q))))
        selected.extend(random.sample(hard_q, min(1, len(hard_q))))
    elif past_score < 80:  # Improving
        # Balanced: 1 easy, 2 medium, 2 hard
        selected.extend(random.sample(easy_q, min(1, len(easy_q))))
        selected.extend(random.sample(medium_q, min(2, len(medium_q))))
        selected.extend(random.sample(hard_q, min(2, len(hard_q))))
    else:  # Mastering
        # Challenge mode: 1 easy, 1 medium, 3 hard
        selected.extend(random.sample(easy_q, min(1, len(easy_q))))
...
if submit:
    ...
    # Track topic performance for ML
    topic_performance = {}
